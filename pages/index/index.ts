...
interface HomeTaskViewModel {
  taskId: string; iconEmoji: string; title: string; subtitle: string;
  status: HomeTaskStatus; indicator: HomeTaskIndicator;
  progress: number; progressMax: number; progressText: string; footerText: string;
  tone: 'green' | 'yellow' | 'blue';
}
interface HomeUiViewModel {
  journey: { title: string; dayText: string; dateText: string; atmosphereText: string; completed: boolean };
  world: {
    state: WorldState; assets: WorldAssetSet; transition: WorldTransition;
    discoveries: WorldDiscoveryView[]; discoveryCount: number;
    companionStage: CompanionVisualStage; companionMood: CompanionMood;
    companionAsset: ResolvedCompanionAsset; companionName: string; companionMessage: string;
    plantText: string; pathText: string; waterText: string;
    energyValue: number; energyMax: number;
    nextUnlockVisible: boolean; nextUnlockTitle: string; nextUnlockDescription: string; nextUnlockImage: string;
  };
  tasks: HomeTaskViewModel[];
  summaryBanners: Array<{ id: string; title: string; subtitle: string; tone: 'green' | 'blue' | 'warm' }>;
}
...
function emptyHomeUiViewModel(): HomeUiViewModel {
  ...
  return {
    ...
    world: {
      state: emptyWorldState(), assets: getWorldAssetSet(0, 0, 0), transition: emptyWorldTransition(), discoveries: [], discoveryCount: 0,
      companionStage: companion.visualStageKey, companionMood: companion.mood, companionAsset: companion.asset,
      companionName: companion.name, companionMessage: companion.message,
      plantText: '0/4', pathText: '0/4', waterText: '0/4', energyValue: 0, energyMax: companion.nextRequiredEnergy,
      nextUnlockVisible: false, nextUnlockTitle: '', nextUnlockDescription: '', nextUnlockImage: '',
    },
    tasks: [], summaryBanners: [],
  };
}
...
Page({
  ...
  refreshAll() {
    ...
    const homeTasks: HomeTaskViewModel[] = taskRaw.map((task: DailyTaskVM) => {
      ...
      const subtitle = task.key === 'meal_any' ? '记录今天任意一餐' : `目标 ${task.target}${task.unitText ? ` ${task.unitText}` : ''}`;
      ...
      return {
        ...
        tone: task.key === 'exercise_min' ? 'yellow' : (task.key === 'water_goal' ? 'blue' : 'green'),
      };
    });
    ...
    const homeUi: HomeUiViewModel = {
      journey: {
        ...
        dayText: planCompletedEntry.visible ? '这一段旅程已经完整走过' : planProgressText,
      ...
      world: {
        ...
        companionMessage: companionVM.transientActive && companionVM.transientMessage ? companionVM.transientMessage : worldState.message,
      ...
      tasks: homeTasks,
      summaryBanners: [
        ...(planCompletedEntry.visible ? [{ id: 'plan', title: planCompletedEntry.text, subtitle: '看看这段旅程发生了什么', tone: 'green' as const }] : []),
        ...(weeklySummaryEntry.visible ? [{ id: 'weekly', title: weeklySummaryEntry.text, subtitle: '看看这一周发生了什么', tone: 'green' as const }] : []),
        { id: 'progress', title: '查看进展', subtitle: '看看这些天发生了什么', tone: 'blue' as const },
      ],
    };
